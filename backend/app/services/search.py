...
class SearchService:
    ...
    def search(self, query: str, limit: int = 3) -> SearchContext:
        """Legacy synchronous search over the current catalog snapshot.

        The snapshot is loaded from Postgres during application startup. The
        synchronous entry point remains available to drafts, scripts and tests
        that used the service before the DB-backed API was introduced.
        """
    ...
    async def search_db(
        ...
        detected_intent = await self._detect_intent_db(session, query, tokens, query_vector)
